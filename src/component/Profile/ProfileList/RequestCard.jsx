...
const ActionButtons = ({ status, openMessageCard, profile, fetchData }) => {
  ...
  const buttonConfig = {
    pending: [
      ...
      },
      {
        icon: <FaRegEye />,
        label: "View",
        onClick: () => handleAction("view", profile._id),
      },
    ...
    accepted: [
      { icon: <RiDeleteBin5Line />, label: "Delete" },
      {
        icon: <FaRegEye />,
        label: "View",
        onClick: () => handleAction("view", profile._id),
      ...
      {buttonConfig[status]?.map(({ icon, label, onClick }, index) => (
        ...
        >
          <span className={styles.actionIcon}>
            {React.cloneElement(icon, { strokeWidth: 1.2 })}
          </span>
